...
function getSurfaceHeightAtPoint(point) {
  ...
  var b = null;
  b = RGB_Terrain.getColor(point);

  var j = NaN;
  if (b !== null)
    j = Math.round(-10000 + (((b[0] * 256 * 256) + (b[1] * 256) + b[2]) * 0.1));
...
map.addControl(new L.Control.Search({
  ...
  moveToLocation: function (latlng, title, oMap) {
    ...
    if (oState.shooting_direction == "between") {
      oEye.setLatLng(latlng);
      oCamera.setLatLng(latlng);
    }
...
var _drawLine = function (sTimeType, sDate) {
  ...
  var bViewFrom = (oState.shooting_direction == "from");
  ...
  } else if (oState.shooting_direction == "from") {
    oEye.setLatLng(End);
  }
...
var drawLine = function () {
  ...
  return _drawLine(sTimeType, sDate, oState.shooting_direction);
}
...
setTimeout(function () {
  ...
  oCamera.on("move", function (evt) {
    //console.log("oCamera.on evt", evt,"oState.shooting_direction",oState.shooting_direction);
...
var resetCurrent = function () {
  ...
  for (var defaultProp in defaultSave) {
    console.log(defaultProp, oState[defaultProp], defaultSave[defaultProp]);
    oState[defaultProp] = defaultSave[defaultProp];
  }
  oState.shooting_direction = "placing";
  oCamera.setLatLng([defaultSave.cameraposlat, defaultSave.cameraposlng]);
  oEye.setLatLng([defaultSave.eyeposlat, defaultSave.eyeposlng]);
  sidebar.open(oState.panel);
  //drawLine();
  setDirty();
}
...
jQuery("#resetCurrent").on("click", resetCurrent)
